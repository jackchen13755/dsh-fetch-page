...
字段约束（提交前本地校验，不满足直接不提交）：

| 字段 | 约束 | 来源 |
| --- | --- | --- |
| `代码变更影响范围`（`impact`） | 不能为空 | 服务端回 `alert('『代码变更影响范围』不能为空。')` |
| `bug详细原因`（`detail`） | ≤ 512 字（按码点计，与 PHP `mb_strlen` 对齐） | 服务端回 `alert('『bug详细原因』长度应当不超过『512』，且大于『0』。')` |

禅道这类校验失败时**仍返回 HTTP 200**，只在响应体里塞一段 `alert('…')` 且 Bug 状态不变；
工具会把这段 alert 原文回显为 `服务端拒绝：…`，并附响应片段，不再只报「状态校验异常」。
`dryRun=true` 时同样会返回 `problems`，可先干跑确认文案是否合规。
