...
function lastResolvedBuild(html: string): string {
  const re = /解决版本[\s\S]{0,300}?旧值为\s*["']([^"']*)["']\s*，\s*新值为\s*["']([^"']*)["']/g
  let m: RegExpExecArray | null
  let build = ''
  while ((m = re.exec(html))) { if (m[2]) build = m[2] }
  return build
}

function extractRequiredFields(html: string): string[] {
  const set = new Set<string>()
  const cfg = html.match(/requiredFields":\s*"([^"]*)"/)
  if (cfg && cfg[1]) for (const f of cfg[1].split(',')) if (f) set.add(f.trim())
  const tdRe = /<td\b[^>]*\bclass=(["'])[^"']*\brequired\b[^"']*\1[^>]*>[\s\S]*?<(?:select|input|textarea)\b[^>]*\bname=(["'])([^"']+)\2/g
  let m: RegExpExecArray | null
  while ((m = tdRe.exec(html))) if (m && m[3]) set.add(m[3])
  const ctrlRe = /<(?:select|input|textarea)\b[^>]*\bname=(["'])([^"']+)\1[^>]*\bclass=(["'])[^"']*\brequired\b[^"']*\3/g
  while ((m = ctrlRe.exec(html))) if (m && m[2]) set.add(m[2])
  const ctrlRe2 = /<(?:select|input|textarea)\b[^>]*\bclass=(["'])[^"']*\brequired\b[^"']*\1[^>]*\bname=(["'])([^"']+)\2/g
  while ((m = ctrlRe2.exec(html))) if (m && m[3]) set.add(m[3])
  return [...set]
}
...
function tagByName(html: string, tag: string, name: string): string {
  ...
  for (const m of html.matchAll(re)) {
    const full = m[0]
    if (new RegExp(`\\bname=(['\"])${name}\\1`).test(full)) return full
  }
  return ''
}

function selectedValue(html: string, name: string): string {
...
function resolveBuildId(build: string, options: SelectOption[]): string {
...
function textareaValue(html: string, name: string): string {
  const re = new RegExp(`<textarea\\b[^>]*\\bname=(['\"])${name}\\1[^>]*>([\\s\\S]*?)<\\/textarea>`, 'i')
  const m = html.match(re)
  return m?.[2] ?? ''
}

function parseForm(html: string): Record<string, any> {
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.tools.register(defineTool({
    name: 'fetch_page',
    description:
      '通过浏览器扩展抓取页面：直接使用浏览器插件桥接（本地守护进程 → 扩展后台 fetch），默认自动携带浏览器 Cookie（绕过 CORS，可访问登录态页面），无需读取或请求 Chrome Cookie 权限。' +
    ...
    async execute(args, _exec) {
      try {
        ...
        return await bridgeForward({
          ...
          mode: typeof args.mode === 'string' ? (args.mode as any) : 'auto',
  ...
  async function forward(method: string, url: string, headers?: Record<string, string>, body?: string): Promise<any> {
  ...
  function maybeThrow(resp: any, label: string): void {
    if (resp?.error) throw new Error(`${label}: ${resp.error}`)
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render(_args, value) {
        if (value.error !== undefined && value.error !== '') return [textBlock(`错误: ${value.error}`)]
        const lines: string[] = []
    ...
    async execute(args, _exec): Promise<any> {
      const a = args as any
      try {
        await ensureDaemon()
        const bugID = String(a.bugID ?? '')
        if (!bugID) return { error: '缺少 bugID' }
        ...
        if (!isLoggedIn(viewHtml)) throw new Error('浏览器未登录 zen.sgrl.io，请确认浏览器已登录后重试。')
        ...
        const submitOnce = async (): Promise<any> => {
          ...
          if (!isLoggedIn(formHtml)) throw new Error('浏览器未登录 zen.sgrl.io，无法打开解决表单。')
          const form = parseForm(formHtml)
          if (!form.uid) throw new Error('解析解决表单失败：未找到 uid（kuid）。')
          ...
          if (a.impact || form.changeImpact) fields.push(['changeImpact', String(a.impact || form.changeImpact || '')])
          if (a.comment) fields.push(['comment', String(a.comment)])
          if (a.detail) fields.push(['detail_reason', String(a.detail)])
