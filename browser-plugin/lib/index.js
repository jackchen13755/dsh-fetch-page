...
function buildVerifyScript(specPath) {
...
export function apply(ctx, config = {}) {
    ...
    ctx.tools.register(defineTool({
        name: 'verify_page',
        description: '打开真实 Chrome 页面，按 JSON 断言契约验证开发结果（元素存在/可见、文本、数量、属性、样式），' +
        ...
        output: {
            ...
            render(_args, value) {
                if (value.error !== undefined)
                    return [textBlock(`验证失败: ${value.error}`)];
                ...
                if (consoleErrors.length > 0) {
                    ...
                    for (const c of consoleErrors.slice(0, 10))
                        lines.push(`- [${(c && c.level) ?? 'error'}] ${(c && c.text) ?? ''}`);
                }
                if (networkErrors.length > 0) {
                    lines.push('');
                    lines.push(`网络请求失败 (${networkErrors.length}):`);
                    for (const n of networkErrors.slice(0, 10))
                        lines.push(`- ${(n && n.errorText) ?? ''} ${(n && n.url) ?? ''}`);
                }
        ...
        async execute(args, _exec) {
            ...
            try {
                ...
                try {
                    await run(`rm -f "${specPath}" "${pyPath}"`);
                }
                catch { /* noop */ }
